	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "select table column where string" {
				if got.TableName != "users" || len(got.Columns) != 1 || got.Columns[0] != "name" || got.Where == nil || got.Where.Left != "name" || got.Where.Operator != "=" || got.Where.Right != parser.StringValue("bob") {
			...
			if tc.name == "select table star" || tc.name == "select table star mixed spacing" || tc.name == "select table star where int" {
				if tc.name == "select table star where int" {
					if got.TableName != "users" || got.Columns != nil || got.Where == nil || got.Where.Left != "id" || got.Where.Operator != "=" || got.Where.Right != parser.Int64Value(1) {
