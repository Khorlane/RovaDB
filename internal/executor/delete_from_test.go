...
func TestExecuteDeleteIntWhere(t *testing.T) {
	...
	affected, err := executeDelete(&parser.DeleteStmt{TableName: "users", Where: where(parser.Condition{Left: "id", Operator: "=", Right: parser.Int64Value(1)})}, tables)
	if err != nil {
		t.Fatalf("executeDelete() error = %v", err)
	}
...
func TestExecuteDeleteStringWhere(t *testing.T) {
	...
	affected, err := executeDelete(&parser.DeleteStmt{TableName: "users", Where: where(parser.Condition{Left: "name", Operator: "=", Right: parser.StringValue("bob")})}, tables)
	if err != nil {
		t.Fatalf("executeDelete() error = %v", err)
	}
...
func TestExecuteDeleteUnknownWhereColumn(t *testing.T) {
	...
	_, err := executeDelete(&parser.DeleteStmt{TableName: "users", Where: where(parser.Condition{Left: "email", Operator: "=", Right: parser.StringValue("bob")})}, tables)
...
func TestExecuteDeleteNoMatchesLeavesRows(t *testing.T) {
	...
	affected, err := executeDelete(&parser.DeleteStmt{TableName: "users", Where: where(parser.Condition{Left: "name", Operator: "=", Right: parser.StringValue("sam")})}, tables)
	if err != nil {
		t.Fatalf("executeDelete() error = %v", err)
	}
...
func TestExecuteDeleteWithOrWhere(t *testing.T) {
	tables := map[string]*Table{
		"users": {Name: "users", Columns: typedCols(), Rows: [][]parser.Value{
			{parser.Int64Value(1), parser.StringValue("alice")},
			{parser.Int64Value(2), parser.StringValue("bob")},
